# File: ace.py — SolisAce模块 for Klipper
...
try:
    ...
    raise ImportError("The 'pyserial' library is required for SolisAce module. Please install it using 'pip install pyserial'")


class SolisAce:
    """
    SolisAce模块 for Klipper
    ...
    def _register_gcode_commands(self):
        commands = [
            ...
            ('ACE_SET_SLOT', self.cmd_ACE_SET_SLOT, "ACE设置料槽耗材元数据"),
            ('ACE_SAVE_INVENTORY', self.cmd_ACE_SAVE_INVENTORY, "ACE保存耗材库存"),
        ]
...
# ==========================================================================
# ACE 温度传感器
# 原 extras/temperature_ace.py 已并入本文件。
# 它把 ACE 设备的干燥腔温度（从 ace._info['temp'] 读取）注册为 Klipper 标准温度
# 传感器，可在 Mainsail/Fluidd 中显示。传感器工厂由下方 load_config 随 [ace] 一并
# 注册，因此配置中【无需】再单独写 [temperature_ace] 段。
# ==========================================================================
...
class TemperatureACE:
    ...
    def __init__(self, config):
        ...
        self.ace = None
        # MCU 引用缓存（在handle_ready中获取，避免每次采样重复查找）
        self.mcu = None
    ...
    def handle_ready(self):
        ...
        # 缓存 MCU 引用（每次采样回调都要用，避免重复 lookup_object）
        try:
            self.mcu = self.printer.lookup_object('mcu')
        except Exception as e:
            logging.warning(f"ACE temperature sensor: Could not cache mcu reference: {e}")
            self.mcu = None
    ...
    def _sample_ace_temperature(self, eventtime):
        ...
        # 如果设置了回调，则调用温度回调
        if self._callback and self.mcu is not None:
