...
```
 ...
 [缓冲0]----[缓冲1]----[缓冲2]----[缓冲3]   ← 4 个独立缓冲器，每路 2 霍尔 + 1 磁铁
    ...
    |         |         |         |        → feed_assist_count）
    +---------+---------+---------+
            [5 合 1 耗材集线器]             ← 4 路 ACE + 1 路预留（不经过 ACE） = 5 入 1 出
                   |
...
4 路 ACE 通道再加 **1 路预留直供**（不经过 ACE，直接进集线器）在 **5 合 1 耗材集线器**汇为单路输出，经外置**编码器**（ITR20403）计量，最后由导管送入打印机挤出机。
...
```
前置     _ACE_PRE_TOOLCHANGE 调用切刀宏切断耗材
开始     传感器2 有料
Phase 2  ACE + 挤出机同步回退 → 传感器1 清除（耗材退出齿轮）
收尾     ACE 单独回退剩余距离 → 料槽 ready
```
...
```
toolchange_retract_length ≈ 挤出齿轮到喷嘴尖距离
                          + ACE 到挤出机入口的导管长度
                          − 切刀宏已执行的挤出机回退量（cut.cfg 的 retract_length）
```

> 切刀宏已先退一段（如 `retract_length: 32.5`），所以同步回退的起点是切断后的耗材头，而非喷嘴。务必扣除这段，否则会过退。
